...
export function screenContracts(payload,read,spot,now=Date.now()) {
  ...
  const out={candidates:[],rejected:{},status:'WAIT',reason:'',partial:!!payload?.partial};
  ...
  for(const r of payload.results) {
    ...
    const delta=g.delta;
    if(d.expiration_date!==day||d.contract_type!==type||d.shares_per_contract!==100||!/^O:SPY\d{6}[CP]\d{8}$/.test(d.ticker||''))reason='Wrong expiry, direction or contract';
    else if(seen.has(d.ticker))reason='Duplicate contract';
    else if(q.timeframe!=='REAL-TIME'||!Number.isFinite(age)||age<0||age>30000)reason='Quote delayed, missing timestamp or older than 30s';
    else if(![q.bid,q.ask,q.bid_size,q.ask_size].every(x=>finite(x)&&x>0)||q.ask<q.bid)reason='Invalid or empty two-sided quote';
    ...
    out.candidates.push({symbol:d.ticker,strike:d.strike_price,type,expiry:day,bid:q.bid,ask:q.ask,spreadPct:spread*100,delta,volume:r.day.volume,oi:r.open_interest,quoteAt:new Date(quoteMs).toISOString(),premium:q.ask*100});
  }
  out.candidates.sort((a,b)=>a.spreadPct-b.spreadPct||Math.abs(Math.abs(a.delta)-0.5)-Math.abs(Math.abs(b.delta)-0.5)||b.volume-a.volume);
  out.candidates=out.candidates.slice(0,5);
  out.status=out.candidates.length?'SCREENED':'WAIT';
  out.reason=out.candidates.length?'Liquidity filters passed; entry permission remains '+read.gate:'No contracts pass all filters';
  return out;
}
