...
export function render(card) {
  ...
  el.innerHTML='<div style="padding:1rem;background:var(--surface-2);border:1px solid var(--border-strong);border-radius:6px">'+
    ...
    '<details data-id="contracts" '+(expanded.has('contracts')?'open':'')+' style="margin-top:0.8rem"><summary>0DTE contract screen · '+esc(contractScreen?.status||'WAIT')+'</summary><div style="font-size:0.72rem;padding:0.5rem 0">'+esc('Provider: '+(contractPayload?.source||'Unusual Whales')+' · '+(contractPayload?.status||'waiting')+' · '+(contractPayload?.results?.length||0)+' contracts')+'<br>'+esc(contractPayload?.reason||contractScreen?.reason||'Waiting for current SPY candles')+(contractLoading?' · Refreshing…':'')+'<br>Same-day standard contracts · real-time quote ≤30s · spread ≤10% · |delta| 0.35–0.65 · volume/OI ≥100. Sorted by spread, distance from 0.50 delta, then volume. Premium is ask ×100; entry checks still apply.'+(contractScreen?.partial?'<br>Partial provider result; ranking covers received contracts only.':'')+'<br>'+[...(contractScreen?.candidates||[]),...(contractScreen?.reviewCandidates||[])].map(c=>esc(c.symbol)+' · Bid/ask '+c.bid.toFixed(2)+' / '+c.ask.toFixed(2)+' · '+c.spreadPct.toFixed(1)+'% spread · Δ '+c.delta.toFixed(2)+' · Ask premium $'+c.premium.toFixed(0)+' · Quote '+esc(c.quoteAt||'timestamp unavailable — confirm with broker')).join('<br>')+'<br>'+Object.entries(contractScreen?.rejected||{}).map(([reason,n])=>esc(reason)+': '+n).join(' · ')+'</div></details>'+
